...
export type Screen =
  | 'home'
  | 'pick_customer_type' // выбор B2C / B2B на самом первом /start
...
// Тип клиента (см. contacts.customer_type, миграция 6).
...
export type SessionState = {
  ...
  // тип клиента (если уже определён — кэшируем здесь, чтобы не дёргать БД)
...
// ---------------------------------------------------------------------------
export const UI = {
  // Приветствие на самом первом /start, когда тип клиента ещё не выбран.
  // brand — отображаемое имя бренда (берётся из BOT_BRAND_NAME).
  audiencePicker: (name: string | undefined, brand: string, strict = false) =>
    strict
      ? `Здравствуйте${name ? ', ' + name : ''}.\n` +
        `Я — бот «${brand}». Чтобы предложить подходящее меню — подскажите:\n` +
        `вы оформляете заказ для частного дома или для компании / стройки?`
      : `👋 Здравствуйте${name ? ', ' + name : ''}!\n` +
        `Я — бот <b>«${brand}»</b>. Чтобы предложить подходящее меню — подскажите:\n` +
        `вы оформляете заказ для частного дома или для компании / стройки?`,

  homeWelcome: (name: string | undefined, brand: string, strict = false) =>
    strict
      ? `Здравствуйте${name ? ', ' + name : ''}.\n` +
        `Бот «${brand}». Выберите услугу или раздел.`
      : `👋 Здравствуйте${name ? ', ' + name : ''}!\n` +
        `Я — бот <b>«${brand}»</b>. Помогу заказать работы по дому и участку за пару минут.`,
  homeMenu: 'Что вас интересует?',
  homeMenuB2b: 'Чем помочь? Заказы оформляем по договору, менеджер подтвердит цену и сроки.',
